...
//
// Reuses the shared duskdaf::DuskPanel (chrome knob / LED / text / value bubble)
...
class MultiSynthUI : public UI, public duskdaf::ParamHost
{
public:
    //--- duskdaf::ParamHost -----------------------------------------------------
        ...
        : UI(DAF_UI_DEFAULT_WIDTH, DAF_UI_DEFAULT_HEIGHT)
    {
        ...
        // is not safe with this DAF ImGui wrapper — the wrapper captures the scale
        ...
        // until the plugin window is reopened. Fixing it properly needs a DAF /
        // shared-daf change (out of scope here).
        fontSet = duskdaf::loadCrispFontSet(kFontSizes, 8, getScaleFactor());
...
protected:
    ...
    void onImGuiDisplay() override
    {
        ...
        // The DAF DearImGui backend has no VtxOffset support, so a single window
        ...
        // The MOD MATRIX is a modal. DAF's ImGui integration does not render an
        ...
        if (showSaveModal)
        {
            ...
            const duskdaf::ResizeGripState grip = submitGrip(winW, winH);
    ...
    // Per-layer VERTEX CENSUS. The DAF DearImGui backend has no VtxOffset support,
    ...
    duskdaf::ResizeGripState submitGrip(float winW, float winH)
    {
        return panel.resizeGrip(dl, winW, winH, kDesignW, kDesignH);
    }

    // The grip's two side effects, both DAF window calls the shared widget header
    ...
    void applyGrip(const duskdaf::ResizeGripState& grip)
    {
        textInputFocus.update(*this);

        // The DAF-Widgets ImGui backend never forwards ImGui::SetMouseCursor() to
...
private:
    ...
    void accentArc(float cx, float cy, float r, float t, bool bipolar, float anchorT = -1.0f)
    {
        ...
        for (int i = 0; i <= N; ++i)
        {
            ...
            const float ang = duskdaf::DuskPanel::knobAngle(tt);
    ...
    // Chrome knob body (matches duskdaf::DuskPanel's chrome exactly) so the local
    ...
    void drawKnobChrome(ImVec2 c, float R, float t, bool ticks = true)
    {
        ...
        for (int i = 0; i <= 10; ++i)
        {
            const float a = duskdaf::DuskPanel::knobAngle((float)i / 10.0f);
                   ...
                   const char* whyTip)
    {
        ...
        const float ia = duskdaf::DuskPanel::knobAngle(t);
    ...
    // thread, and DAF has no plugin->host parameter notification, so neither the
    ...
    void ghostKnob(uint32_t p, float cx, float cy, float r, float a)
    {
        ...
        const ParamDef& d = kParamDefs[p];
        const float t = (d.max > d.min) ? (values[p] - d.min) / (d.max - d.min) : 0.0f;
        const float ang = duskdaf::DuskPanel::knobAngle(t);
    ...
    // the mod matrix (DAF ImGui can't float an overlapping window). InputText for
