...
protected:
    ...
    void initParameter(uint32_t index, Parameter& p) override
    {
        ...
        // NOTE: DAF honours kParameterIsHidden in the LV2 exporter ONLY; the VST3 and CLAP
        ...
        if (d.kind == 'c' && d.choices != nullptr && d.numChoices > 0)
        {
            ...
            p.enumValues.values = vals; // DAF takes ownership (deletes[] on destroy)
        }
    }
    ...
    // DSP setters are atomic stores — safe from whichever thread DAF uses.
    ...
    // Re-size the DSP scratch buffers when the host changes the block size. DAF
