...
// Generated by dusk-audio-tools plugins/FourKEQ/tests/reference_comparison:
// probe_dense_bands.py -> fit_dense_bands.py -> emit_dense_band_cpp.py.
//
// DO NOT "FIX" THE DUPLICATED FINAL ANCHOR. In every one of the 64 non-constant
// arrays below, value[N-1] and value[N-2] agree to within 2.2e-7 relative. They
// are not all bit-identical (29 of 64 are); the rest differ only in the last
// printed digit, which is what emit_dense_band_cpp.py's "%.8g" formatting does
// to two equal fitted doubles. The adjacent value[N-2]/value[N-3] pair is the
// control: only 3 of 64 land inside that tolerance and the worst differs by
// 15%. So the final pair is a genuine match, not a coincidence of rounding.
// That is NOT an off-by-one in the emitter:
// the probe sweeps 21 distinct normalized positions, the fit runs once per
// captured scenario, and the only duplicating path in the emitter is the
...
// filters (kBrownHPFFrequency ends 303.731659, 303.738747, 303.738747).
//
// The visible consequence is that the top of each knob is inert: the last 1.5 dB
// of gain (+13.5 to +15) and the top frequency segment (above 427 / 2350 / 6700 /
// 15400 in control coordinates). That inertness is what the reference does, so
// reproducing it IS the parity. Extrapolating these anchors was costed at up to
// +45% on one Q anchor and +6 to +9% on every band's top gain anchor, i.e. it
// would move the plugin away from the reference, not toward it.
//
// The arrays are 21 measured points, or 1 synthetic + 10 measured for the
// gain-indexed ones (control 0 dB is not swept; the emitter prepends it), which
// is why those also duplicate at the START.
