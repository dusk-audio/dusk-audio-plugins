...
// Linux behavioural probe for the framework-free DAF CrashLog.
...
bool testPartialUnlink(bool mutant)
{
    ...
    ok = expect(controlAfterClose == 0,
                "fully unlinked DAF module unloads (control measurement)") && ok;
    ...
    ok = expect(afterClose > 0,
                "partial unlink pinned the DAF module across dlclose") && ok;
