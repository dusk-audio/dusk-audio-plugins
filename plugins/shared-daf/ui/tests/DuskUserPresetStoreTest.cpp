...
int main()
{
    ...
    const auto legacyLibrary = temporary.path / "legacy-default-replace";
    const auto legacyFirst = duskdaf::writeUserPreset(
        legacyLibrary, ".preset", "Tape Echo Save",
        [](std::ostream& output) { output << "value=first\n"; });
    const auto legacyReplacement = duskdaf::writeUserPreset(
        legacyLibrary, ".preset", "Tape Echo Save",
        [](std::ostream& output) { output << "value=replaced\n"; });
    check(legacyFirst && legacyReplacement && legacyReplacement.path == legacyFirst.path
              && fileContents(legacyFirst.path) == "name=Tape Echo Save\nvalue=replaced\n",
          "four-argument save preserves Tape Echo's same-name replacement behavior");
