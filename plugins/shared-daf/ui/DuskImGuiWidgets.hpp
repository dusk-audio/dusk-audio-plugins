...
//
// DuskImGuiWidgets.hpp — reusable Dear ImGui panel toolkit for Dusk DAF UIs.
//
// Extracted from plugins/tape-echo/daf-plugin/TapeEchoUI.cpp. Provides a fixed
...
// any DAF include (the UI subclass adapts editParameter/setParameterValue).
//
...
// applying the new size and showing a diagonal mouse cursor -- are both DAF
...
public:
              ...
              void* displayContext = nullptr)
    {
        ...
        // never reset. io.KeyCtrl/KeySuper map to the platform's real modifiers in DAF.
    ...
    // the DAF counterpart of what plugins/shared/ScalableEditorHelper does for the
    ...
    // minimum handed to setGeometryConstraints(designW, designH, true): DAF clamps
    ...
    // read 1.0 under AU. So the scale here, like DAF's own constraint, is in device
    ...
    //  - map `hot` onto a real cursor yourself; the DAF-Widgets ImGui backend has
                               ...
                               float minScale = 1.0f, float maxScale = 3.0f)
    {
        ...
        // absolute pointer delta. setSize() is asynchronous (DAF: "will not change
