...
namespace
{
    ...
    // (paramIndex,value) rows the DAF shell exposes as host programs. The old
...
class MultiQUI : public UI, public duskdaf::ParamHost
{
public:
    MultiQUI()
...
private:
    ...
    // view instead of yanking the user into another skin. Multi-Q's DAF param
    ...
    // mapping, scale factors and fit. JUCE writes normalised APVTS values; the DAF
    ...
    // omitted: DAF param edits are host-undoable, so there is no internal stack.
    ...
    void drawDigital(ImDrawList* dl)
    {
        ...
        for (int b = 0; b < 8; ++b)
        {
            ...
            // within 0.5 dB of 0), converted to the DAF design (×960/1050 ≈ 11 px base).
                    ...
                    ImU32 capCol, const char* label, int fmt, bool dim)
    {
        ...
        // 12-o'clock centre; unipolar params fill from the start.
        {
            ...
            const float aS    = duskdaf::DuskPanel::knobAngle(0.f);
            const float aE    = duskdaf::DuskPanel::knobAngle(1.f);
            const float aV    = duskdaf::DuskPanel::knobAngle(t);
            const float aMid  = duskdaf::DuskPanel::knobAngle(0.5f);
    ...
    void drawDetailPanel(ImDrawList* dl)
    {
        ...
        // ---- EQ knobs ----
        detailKnob(dl, "dafreq", 128, cy, R, mqidx::freq(b), 20.f, 20000.f, true, col, "FREQ", FMT_FREQ, false);
    ...
    void drawDigitalHeader(ImDrawList* dl)
    {
        ...
        // Save — VISUAL ONLY (no user-preset store wired in the DAF shell yet).
    ...
    [[maybe_unused]] void drawDigitalToolbar(ImDrawList* dl)
    {
        ...
        headerButton(dl, "dafreeze", 754, y0, 810, y1, digFrozen_ ? "FROZEN" : "FREEZE",
    ...
    float tubeMidDb(float freq, double sr) const
    {
        ...
        auto peakMag = [&](float fc, float q, float gainDB) -> double {
            ...
            // Track the TUBE AUDIO path's own ceiling, not the shared-DAF one.
    ...
    void drawCredits(ImDrawList* dl, float winW, float winH, bool blockAction)
    {
        ...
        const auto& tiers = duskdaf::patreonTiers();
    ...
    // FFTs on the UI thread (shared duskdaf::RealFFT), smooths in dB across frames,
