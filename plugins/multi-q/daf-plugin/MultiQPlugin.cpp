...
//
// MultiQPlugin.cpp — DAF shell around the framework-free MultiQDSP core.
//
...
public:
    ...
    bool matchComputeForUI() noexcept
    {
        ...
        // updateLatency() reports any Match-FIR latency change (DAF contract: setLatency
...
protected:
    ...
    void initParameter(uint32_t index, Parameter& p) override
    {
        ...
        if ((d.kind == 'c' || d.kind == 'b') && d.choices != nullptr && d.numChoices > 0)
        {
            ...
            p.enumValues.values = vals; // DAF takes ownership
        }
    }
    ...
    void sampleRateChanged(double newSampleRate) override
    {
        ...
        // left to run() (DAF contract — not called from this lifecycle path).
        if (!matchStateStr.empty())
            dsp.matchProcessor().deserialize(matchStateStr);
    }
