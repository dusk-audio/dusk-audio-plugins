...
void DuskVerbEngine::prepare (double sampleRate, int maxBlockSize)
{
    ...
    // #123 stereo-image env hooks (offline measurement harness only; message
    // thread → getenv is safe here; unset → bit-null):
    // DUSKVERB_STEREOBIAS = per-engine levers, DUSKVERB_POSTSTEER = post-tank steer.
