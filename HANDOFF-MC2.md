## Partial FET programme release fix installed — 2026-09-11

See [the FET release report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-fet-programme-release-2026-09-11.md).
Evidence: `build-multi-comp-1176/fet-programme-20260911/` (E).

- FET 400 ms error median/p90/worst: **4.636/9.719/11.088 → 3.179/5.276/8.651 dB**.
  Across 180 programme comparisons, 54 improve and 126 retain the same level
  maxima; none worsens at 400/50/10 ms. **Still 0/60 passing the full criterion.**
  Two balance and five band comparisons worsen; largest increases .143/.482 dB.
- The accepted correction adjusts the deep fast-release control and detector,
  reaching their fast floors at normalized Release .9. The depth transition
  preserves the existing shallow knee. Exposure/slow populations remain unchanged.
  Charge-only exposure was rejected: retained native tail error .271 > .150 dB.
- New native regression fixture: 32 rows/128 samples, old maximum 19.050 → 3.565 dB,
  bound <4 dB. This is a partial-improvement guard, not an audibility claim.
  Native fixtures/tolerances remain unchanged elsewhere; only internal H3 model
  snapshots/coordinates were regenerated. Six deliberate fault families fail;
  restored checks pass. Shallow music identity is a local evidence check.
- Earlier named holdouts were inspected during selection. Three fresh settings
  were captured after final DSP freeze: all 15 RMS/max comparisons improve,
  pooled RMS 5.597 → 1.399 dB. Fresh All-buttons still has up to 12.645 dB error
  in the earliest recovery bins. No later DSP fitting used those scores.
- Non-FET matrix: **168/168 byte-identical**. Installed VST3 vs final core:
  **9/9 programme renders sample-identical**. Offline FET CPU approximately +3%.
- Full CTest **16/16, 400.47 s**; auval and pluginval strictness 5 pass. Built and
  installed AU/VST3/CLAP/LV2 manifests match; strict AU/VST3/CLAP signatures pass.
  Installed AU SHA: `a733caa63144a0730ecf598a9bc3e7775ba8ac64e637b0cb70838a883d871e83`.
- Passing native gates that worsened: stereo .009922 → .014354 (<.020), fifth
  harmonic .001763 → .043954 (<.250). Dense quiet recovery stays .140045 (<.150).
- Runner moved to `E/archived-capture-runner/`; no evidence deleted. About 1 GB
  added, 18 GiB free. No commit. BUS remains held for ABX; OPTO/VCA unchanged.

Next: FET duration/history and middle-release behavior, early All-buttons and
remaining spectral/level errors; then the previously measured VCA and OPTO gaps.
The report records rejected trials, exact scopes, integrity checks and limitations.

## OPTO/FET/VCA programme candidates compared: none at parity — 2026-09-11

See [the comparison report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-programme-candidate-2026-09-11.md).
Evidence: `build-multi-comp-1176/programme-candidate-20260911/` (180 installed-VST3
captures, `compare.py`, `scores-v2.json`) and `programme-fix-20260911/` (core lab,
falsifier cores, band-limited native probe). Every OPTO and FET setting and 58/60 VCA
settings exceed the audibility criterion; median worst 400 ms level error OPTO 1.98,
FET 4.64, VCA 0.21 dB (max 4.94 / 11.09 / 15.71). Static/tone gates still hold; programme
dynamics were never gated.

Metric v2 (declared before use): 20 kHz band-limit on both sides (the bundled snare
has 14 % of its energy above 20 kHz, which our 2× filters remove) and a
resolution-corrected band check (the old one scored inaudible DC at 2.37 dB). BUS
re-scored with v2 in `bus-audible-v2-20260911/`: worst 400 ms 0.454 → 0.424 dB, so
the ABX set stands.

Mechanisms tested with the byte-exact lab (renders identical to the installed VST3):
FET programme release ~3× too slow (exposure pinned: partial, 4.64 → 3.19 dB; slow
populations: falsified); VCA SC-On tilt over-responds above 20 kHz (confirmed on a
native band-limited re-capture: 3.3–3.7 → 0.4–0.7 dB) plus an onset clamp from silence
(first 10 ms 13.9 dB low); OPTO attack too slow / release too long at depth plus a
−0.6 dB 16 kHz shelf. Recommended order: FET release, VCA >20 kHz tilt, onset from
silence (VCA/OPTO), OPTO depth dynamics, then BUS per ABX. No DSP/test/tolerance change,
no install, nothing committed or deleted. (2026-09-11 cleanup: campaign reports and
BUS handoffs now live in `dusk-audio-tools/plugins/MultiComp/handoff/reports/`; test programs live in
dusk-audio-tools, never in this repo.)

## OPTO/FET/VCA native programme grid captured — 2026-09-11

See [the capture report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-programme-native-2026-09-11.md).
Evidence: `build-multi-comp-1176/programme-native-20260911/`.
**180 native settings (60 per mode), 540 programme WAVs, 1,080 total WAVs**
completed and passed a full reread/hash/readback audit. `programme-index.csv`
indexes the three programmes; `grid.json` records exact settings;
`captures.json` contains the complete validated metadata.

Grid data: **3.5469 GiB**, **529.37 s** render/validation time. Whole evidence
directory including failed attempts and controls: about **3.76 GiB**; final
free space **22.82 GiB**. No old evidence deleted. Frozen input WAVs, renderer
binary hash, native bundle fingerprints, command arrays and logs are retained.

Repeated pilots and 5/10-second preroll controls are sample-identical for all
three programmes in each mode. Sixteen deliberately injected integrity faults
fail, with the restored checks passing. Some fractional UAD requests read back
one Q16 step lower; final grid positions use stable binary values and verified
central/default positions. The 2e-6 readback tolerance is unchanged; the final
worst error is **4.84375e-10**. Four failed request attempts remain as evidence.

**128 programme WAVs contain float samples above unity** (OPTO 86/FET 38/VCA 4),
flagged in `summary.json` without normalization or clipping. This alone does
not establish internal reference clipping. Native versions: LA-2A 1.0.8,
1176 1.0.3, dbx 160 1.0.9. Every battery has full parameter readbacks, Power on,
48 kHz stereo, block 512, preroll 5 s, no automation or external stem.

DSP/test hashes and installed Dusk AU `bc82d0b8…` remain unchanged. No candidate
survey, fitting, tolerance change, commit or repeat installation. BUS fixes
remain on hold pending the owner's ABX results; these native references are
ready for the OPTO/FET/VCA candidate comparisons.

## BUS programme ABX ready; transparency unproven — 2026-09-11

See [the listening report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-abx-2026-09-11.md) and open
`build-multi-comp-1176/bus-abx-20260911/BUS-blind-listening.html` in a browser.
Six cases: two snare, two session mix, two piano; 16 blind trials per case,
synchronized switching, preserved relative levels, local notes/progress and
JSON export. Listener results are pending. Keep the separate private source
key unread before listening. No DSP or tolerance change in this pass.

Fresh installed-AU renders match the core with max sample difference **0.0**
for all six selected cells. The 200 ms absolute gaps are **0.454471 / 0.313298 /
0.368095 / 0.306151 / 0.345654 / 0.284201 dB**. The worst snare is quieter in
the candidate, correcting the supplied description. Its source is only 160 ms,
so the 200 ms number integrates a hit rather than measuring sustained sound.
After both invalid comparisons are excluded, the survey has **252 captures
but 163 unique parameter tuples**. Duplicate captures weight its percentiles.

Nine audio/export faults, five trial-state faults and four browser/export faults were
observed to fail, then the restored versions passed; see the report and logs.
Chrome decoded every pair and verified the real audio gain routing. No human
audibility result follows from these software checks. Low ABX scores alone do
not prove inaudibility or justify loosening gates.

OPTO/FET/VCA reference AUs exist, but no complete native programme battery was
identified in their archives. The shared capture harness retains only the
requested stem and deletes the extra battery. These surveys need fresh native
programme captures; only a read-only inventory was completed here.

All 33 recorded DSP/test hashes and installed AU `bc82d0b8…` are unchanged.
No commit, evidence deletion, repeat plugin install or new DSP fitting.

## BUS complete model promoted; Auto shape metric repaired — 2026-09-11

See [the finish report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-finish-2026-09-11.md). Evidence:
`build-multi-comp-1176/bus-parity-finish-20260911/`. **Promoted and installed:**
the preceding pass's complete 4:1/10:1 law, native drive ceiling, detector gain
exponent, rederived attack tables and meter factor. Auto poles are unchanged.
Nothing is committed; unrelated working-tree changes and old captures remain.

All five witness errors are now **0.155151 / 0.096962 / 0.041290 / 0.229766 /
0.269220 dB**, with bounds retightened to measured + 0.01. The 27 previously
sealed holdouts reproduce H1/H2/H3 maxima **0.211755 / 0.297763 / 0.191737 dB**.
No new fitting or holdout-based selection occurred.

The old Auto shape gate rejected an exact two-pole decay: 0.048940 > 0.04 dB,
because its late-point extrapolation had weights up to 34.114. The replacement
fits all seven samples to the fixed native poles plus a constant and compares
residuals (< 0.01 dB), requiring both decay components to be active. Complete
model residual **0.001329 dB**. Wrong fast/slow poles, the old switched reservoir,
and unity output each fail; an exact two-pole control passes. See the report for
the native extraction and every fail/pass output. Both model snapshots were
regenerated with failing old references and passing restored references.

Full CTest **16/16, 376.97 s**; auval and pluginval strictness 5 including editor
tests pass. All four installed bundle manifests match; AU/VST3/CLAP strict
signatures pass. AU SHA-256:
`bc82d0b8e9e96589f6c73e2e2a09d405db9ecc26284ff2daf185e62255086721`.
The fresh matrix keeps **168/168 non-BUS slices byte-identical** (`d4f81981…`).

Tradeoffs: active BUS CPU **+29.53% versus the partial model** in this pass's
offline benchmark (23.03–36.30% by cell; complete model 2.52–2.80% of one core).
2:1 transient max 0.185863 → 0.186935 dB; 10:1 meter window
0.519440 → 0.546137 dB; compressed-ceiling peak error 0.520403% → 0.659481%.
All remain inside unchanged limits; the report includes RMS/raw meter values
and the improved audio metrics. No CPU optimization was attempted.

Build pitfall: copying isolated headers with `copy2` retained old timestamps,
so the first incremental build mixed old DSP objects with newly compiled tests
and crashed. Those results are invalid and retained as `stale-*`/initial logs.
Fresh header timestamps forced recompilation; all final results above are from
that rebuilt source. Use fresh timestamps when promoting files. A deliberately
retained detector-gain cache did not trip existing 2× reset probes; that coverage
limit is documented, while production still resets the cache through BusState{}.

Still outside this pass: separate P2/4× output-clamp work, x86 robustness,
Linux/Windows validation and interactive Logic listening. Disk remains 98% full,
about 22 GiB available. No evidence deleted, DAW quit, version bump or publication.

## BUS recalibration pass 2: 2:1 fixed; complete model awaits a gate decision — 2026-09-11

See [the pass-2 report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-recal2-2026-09-11.md). Evidence:
`build-multi-comp-1176/bus-recal2-20260910/` (`REPORT.md` is the ledger).
**Promoted (owner-approved partial model):** 2:1 upper-law nodes (over 24..40,
linear continuation) and a release-dependent charge offset
(`busReleaseChargeOffset`, zero at 0.1 s). Witnesses:
- 2:1 / 30 ms / 1.2 s: 3.761999 → **0.157355**
- 2:1 / 30 ms / 0.1 s: 8.539519 → **0.500530**
- 4:1: 0.952980 → 0.864339
- 10:1 / 1.2 s: 0.532234 → 0.480359
- 10:1 / 0.1 s: 0.710149 (unchanged)

Bounds were re-tightened to measured + 0.01. Bass fixed 0.186 → 0.179; all other
§3 gates unchanged. Two gates regressed within tolerance:
- External-rate worst 0.429 → 0.459 (< 0.5; RMS improved 0.189 → 0.167).
- Meter 10:1 window 0.505 → 0.519 (< 0.6; audio got more accurate there, and the
  display factor is unchanged by its own re-derivation).

Snapshots regenerated with fail/pass proofs (golden BUS RMS 0.020885564,
`externalHandoff`); four deliberate faults each fail a gate. CTest **16/16,
364.36 s**; non-BUS matrix 168/168 byte-identical; auval and pluginval 5 pass;
installed AU `c1d9947b…`, strict signature OK.

**Not promoted:** a complete model closes 4:1/10:1 too: all witnesses ≤ 0.27,
sealed H1/H2/H3 ≤ 0.298, 32/33 gates. It adds:
- 4:1 nodes and a 10:1 scale.
- The native 128.86 dB drive ceiling.
- A detector gain exponent k = 1.0213. With it, the onset-derived attack RCs match
  native saturated-square RCs.
- Re-derived attack tables and meter factor 0.917.

It fails only `--storage-shape` (0.051 vs < 0.04). That gate's native reference is
×34-amplified ≤ 0.0018 dB noise; native recoveries are pure two-pole. **Owner
decision:** change that metric, then promote from `cores/prod-final` via
`make_prod.py`. Do not change Auto poles to pass it. Nothing committed.

## BUS recalibration stopped; larger gap measured — 2026-09-10

See [the recalibration report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-recalibration-2026-09-10.md).
**Production DSP retained; full BUS parity remains open.** The earlier closeout
held only within its measured regimes. The fresh 45-render grid and five
additional controls expose 8.539519 dB of under-compression at 2:1 / 30 ms /
0.1 s, threshold −15, +15 dBFS input. A native repeat is byte-identical; makeup 0
still gives 8.545731 dB. This corner has no new permanent witness yet.

S1 reproduces all four old witnesses within 1e-6 and supports excess
m-sensitivity. Both native repeat controls match. The first isolated concave
conductance trial, u_s=30 dB with rederived attack tables, fails unchanged
native gates: attack 1.269510, independent Auto 1.943365, bass fixed 1.086278,
bass Auto 0.503977 dB. Baselines are 0.415354 / 0.586159 / 0.186266 / 0.373584.
The handoff's “any currently passing gate fails” stop rule was applied. Do not
install the experimental source or infer rejection of the entire formulation:
45 dB has only a partial fit; larger ladder values are untested. Six of twelve
30 dB attack fits reached the original 12-iteration cap; see convergence flags.
No S3–S6, P2, or shared/sibling robustness edits were reached. H1–H3 were recorded
at baseline only and never used for fitting or experimental selection.

S0 CTest: 16/16 pass, 352.92 s; final: 16/16, 349.91 s. Auval, pluginval
strictness 5 including editor tests, and installed AU strict signature all pass.
All four installed manifests match the build; AU SHA remains `4bfbe945469bfab8…`.
All 1,067 printed BUS measurements match S0. Final matrix is byte-identical on
all 240 configurations (168 non-BUS, 72 BUS). No production DSP/test/fixture/
tolerance change; this report and handoff entry are the only production-tree edits from this pass. Evidence:
`build-multi-comp-1176/bus-recal-codex-20260910/`. Existing evidence is read-only;
no cleanup, commit, version bump or DAW manipulation was performed.

## BUS measured parity complete — 2026-09-09

See [the BUS completion report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-completion-2026-09-09.md).
This supersedes the outstanding BUS work in the historical entries below.
**Accepted and installed:** independent external VCA detector with matched FIR
oversampling; calibrated internal/external SC filter; clean output path with
headroom-scaled saturation after makeup; bounded hot charging; audio-clock BUS
meter mechanics and calibration. Raw GR telemetry and the rack size are retained.

Measured worst errors: external level 16.365448 → 0.142705 dB; external
frequency/filter 20.616953 → 0.288869 dB; internal filter 5.847407 → 0.263273 dB;
HR4 makeup 6.729639 → 0.006584 dB; external timing 13.684683 → 0.585125 dB;
independent rate bursts 1.768079 → 0.429460 dB. The original internal Auto gate
still passes at 0.586158 dB worst (slightly worse than storage's 0.577796).
Saturation matrix: 0.041726 dB level error, 0.935876% peak error, clipped H3
0.220743 dB worst difference. Mix 0/50/100 holdout max errors:
0.000121 / 0.058584 / 0.091948 dB.

Meter: second-order 24 rad/s, damping 0.7, gain 0.92, designed per sample rate;
position/velocity cleared in prepare/reset/mode exit. 4:1 and independent 10:1
native traces: RMS 0.178125 / 0.182633 dB. Native UI packet timing requires the
reported ±25 ms maximum-error window (0.123530 / 0.505428 dB); instantaneous
maxima are 2.236429 / 1.193816 dB. See the report before interpreting these as
sample-accurate motion. Offscreen WKWebView rendering was solved with an isolated
UI clock; accepted continuous observations avoid screenshot pauses.

Full final CTest **12/12 pass, 331.58 s**. Auval and pluginval strictness 5 including
editor tests pass. AU/VST3/core match exactly in five independent integration
renders; all five are byte-identical before/after the meter addition. Seven
sibling modes remain byte-identical in 28 mono/stereo/internal/external checks.
Lifecycle, filter ownership, rate, routing and rendering faults all fail their
intended guards in isolated builds; restored versions pass. Full outputs are
linked in the report. No existing audio-parity threshold was widened.

AU SHA-256: `397fd0692990ab8c0709d7eea346705025ab166bfa5348be5bdabaa3ae3ffa99`.
All four installed manifests match; AU/VST3/CLAP strict signatures verify after
re-signing stale VST3 resources. Artifacts:
`build-multi-comp-1176/bus-completion-20260909/`; `probe-production` is final,
`probe-before-meter` is the accepted audio build before display integration.
Do not rerun `setup.py`: it would overwrite the original baseline snapshot.

Completion means the documented measured tolerances, not universal or bit-identical
UAD emulation. No interactive Logic listening, Linux/Windows validation, version
bump, commit or publication. Logic was left running. Historical OPTO/FET/VCA
residuals remain documented; their production audio is unchanged by this pass.
Concurrent Chord Analyzer and other unrelated work is preserved.

## BUS continuously coupled Auto control — 2026-09-09

See [the current BUS storage report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-storage-2026-09-09.md).
**Accepted and installed:** Auto now advances two continuously coupled control
states with measured 41.65904 ms / 4.1588435 s poles and 86.82% sustained slow
recovery. This fixes the switched reservoir's early-recovery shape. The same
−0.33 dB/s bias measured for fixed release now predicts Auto's quiet drift too.
No attack/static-law, UI, preset, fixed-release or coloration change.

Independent Auto 270-point RMS/max: 0.185323/0.619217 → 0.151954/0.577796 dB.
44.1/96 kHz max: 0.617291 → 0.503058 dB. Fresh 120-point storage max:
0.587466 → 0.567863 dB. All three Auto programme RMS/max/absolute level errors
improve; 10:1 frame max 0.255303 → 0.142167 dB. Bass Auto max
0.385883 → 0.371270 dB. Quiet max 0.110857 → 0.009296 dB; early-recovery
extrapolation residual 0.866932 → 0.031671 dB (a shape metric, not absolute GR).

Six coefficients are designed at host rate in prepare/rate updates. No new
signal history: existing fixedControlDb/autoSlowDb reset through BusState{};
external/fixed handoffs clear/synchronize them. New quiet and recovery-shape
guards fail on baseline, pass restored. Independent bound tightens .7 → .6 dB,
failing baseline at .619207 and passing restored at .577795. Wrong-rate and
external-coupling history faults fail; restoration passes. The external handoff
test now uses a captured pre-change external response because internal Auto's
new quiet bias invalidates its old equal-history assumption. Older Auto tests
now match native five-second preroll; lifecycle tests stay cold.

Full CTest 10/10 pass (288.69 s), followed by recompilation/rechecks of the final
Auto test setup and a final BUS Dynamics pass (53.90 s). Auval and pluginval
strictness 5 including GUI succeed. All
four installed manifests match and AU/VST3/CLAP signatures verify. Five installed
AU/VST3/core renders are sample-identical. Seven sibling modes, 36 external
cases, fixed programme, mono/stereo bypass/dry/listen remain byte-identical.

AU SHA: `12597068c4bee911b974ebe9f3374ef26bb3e6cc12900cc307c3a0aaa92922ef`.
Artifacts: `build-multi-comp-1176/bus-storage-20260909/`; use `probe-production`.
`gate*` and `peak*` models are rejected controls; `linear-bias` became the
accepted model, with coefficients cached in production. Logic was left running.
No commit or version bump; concurrent chord-analyzer work preserved.

**Full parity remains open.** Fastest 2:1 independent max worsens .201334 →
.239133 dB (RMS .090786 → .092430); fastest 4:1/10:1 maxima also increase.
Retain these, .577796 burst max, +.044028 dB 2:1 programme offset, quiet
rate-dependent audio offset, fixed/nonlinear/external/meter/fade limits.

## BUS Auto quiet-control investigation — 2026-09-09

See [the Auto control investigation](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-auto-control-2026-09-09.md).
**No production change accepted; the feedback-coupling AU below stays installed.**
Fresh quiet UAD Auto captures measure about +0.10 dB gain after a long hold;
current model error reaches 0.110857 dB. The existing fast/slow RCs fit the
cold 48 kHz drift with 0.000009741 dB maximum residual. Warm, changed-level/ratio
and 96 kHz holdouts support the drift; exact circuit/charging topology remains
unproven. Fresh original Auto reference repeat is byte-identical, sample delta 0.

Signed control precision alone does not fix the drift. Isolated fast/slow bias
variants improve quiet gain (best maximum 0.009328 dB), but worsen all three
Auto programme comparisons. The raw-target variant fails the existing bass
guard: 0.408506 dB, exit 1; retained production passes at 0.385883 dB, exit 0.
Refitting only storage to 0.891033 improves original training RMS but worsens
independent burst RMS 0.185323 → 0.194627 dB and all programme RMS values.
Do not promote these probes or infer success from their quiet/training scores.
Measure storage charging independently across pulse level/duration and bass
content before another shared-fraction adjustment.

No production source, test assertion/tolerance or plugin bundle changed.
BUS Dynamics/Programme rerun: 2/2 pass, 42.18 s. Full core snapshot is unchanged;
all four installed manifests match builds and strict AU/VST3/CLAP signatures
verify. AU SHA remains `8aaa65038cca643efcb44a98edfd21f49e84ddc116d2dc41084acd8267f4630f`.
No installation, DAW restart, commit or version bump. Artifacts, controls,
rejected probes and scripts: `build-multi-comp-1176/bus-auto-control-20260909/`.
Use `baseline/probe` there for the current production model. The `bias-*` and
`probe-fit` variants are rejected experiments. Full BUS parity remains open.

## BUS feedback coupling continuation — 2026-09-09

See [the current BUS programme report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-programme-2026-09-09.md).
The internal compressed feedback signal now has a **1.6666667 Hz coupling pole**,
using the shared DCBlocker. The original 10:1 fixed programme frame maximum
improves **0.859614 → 0.299757 dB**, RMS **0.308720 → 0.106782 dB**. Its Auto
maximum improves **0.518854 → 0.255303 dB**. All six original programme frame
RMS/max metrics improve; overall level offsets increase in several cases,
reaching **+0.063031 dB**. Do not describe this as uniformly improved parity.

Control experiments excluded stereo linking and a stronger exponential charge
curve as the main cause. The full output-transformer feedback tap improved bass
but failed the existing 4× phase/detune guard at 0.424509 dB. Coupling alone
preserves the bass improvement and passes that guard. No static-law coefficient,
attack/release/bias constant, parameter, UI, coloration or golden oracle changed.

Fresh asymmetric-bass fixtures cover 44.1/48/96 kHz, all three ratios, Auto,
unequal stereo and mono. The stereo 10:1 fixed maximum improves
0.352076 → 0.177656 dB. All fixed cases, including mono 1×, improve
0.379800 → 0.204947 dB; the new guard is explicitly **0.22 dB**, since its initial
0.2 dB attempt failed on mono 1×. New Auto maximum improves
0.458700 → 0.385883 dB (0.4 dB guard). Existing guards remain unchanged.

Two fixed filter histories reset in prepare/clearBusState; runtime rate changes
update their poles without clearing history. External processing keeps them
current for internal reentry. Baseline/fault checks fail and restoration passes:
reset/prepare sample difference 0.002693572 → 0; external reentry
0.030455962 → 0; wrong fixed-rate coefficient max 0.587778 → 0.204947 dB.

Full CTest: **10/10 pass, 263.06 s**. Auval succeeds; pluginval strictness 5,
including GUI, reports SUCCESS. All four formats installed; complete manifests
match and signatures verify. Installed AU/VST3/core bass and fixed/Auto
programme renders match sample for sample. Seven native sibling modes (including
OPTO/FET/VCA), 36 external cases and mono/stereo bypass/dry/listen comparisons
are byte-identical to baseline. Internal Auto changes intentionally.

Installed AU SHA-256:
`8aaa65038cca643efcb44a98edfd21f49e84ddc116d2dc41084acd8267f4630f`.
Artifacts: `build-multi-comp-1176/bus-programme-20260909/` (baseline, captures,
rejected probes, fault evidence, native checks and manifests). Frequency scores
are `frequency-holdout-scores.json`; final fresh bass scores are
`coupled-scores.json`; full-output-tap experiment scores are
`output-tap-holdout-scores.json`. Match five-second preroll for reference parity;
native/core identity checks use zero preroll on both sides.

**Full BUS parity remains open.** Preserve the independent Auto burst maximum
0.619207 dB, fixed-level maximum 0.598298 dB, programme/level offsets above,
external UAD calibration, nonlinear waveform/saturation and meter/fade limits.
The fresh 4:1 bass maximum slightly worsens 0.163552 → 0.177240 dB. Static
72/171-cell maxima are 0.225686/0.222533 dB; frequency holdout max 0.439641 dB.
Logic was left running; save/restart to load the binary. No commits, version
bump or publication; concurrent chord-analyzer changes were preserved.

## BUS fixed-release control continuation — 2026-09-09

See [the current BUS release report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-release-2026-09-09.md).
Fresh UAD captures identify a release-dependent below-threshold gain bias.
The model now uses **0.33 × release-RC dB**, shared by all ratios, and retains
signed fixed control in dB to avoid near-unity float quantization. Independent
neutral-gain maximum improves **0.144374 → 0.008431 dB**; recovery-shape maximum
**0.141714 → 0.008372 dB**; fixed-release switching **0.139604 → 0.003725 dB**.
New fixtures span all fixed releases, 44.1/48/96 kHz, mono/stereo, plus 1×/4×.

The new scalar resets through `BusState{}`; Auto/external paths synchronize it
before returning to fixed release. Reset, repeated prepare, both handoffs,
precision and release-switch faults all demonstrably fail the new guards and
pass when restored. The existing phase test needed equal control histories:
silence now advances bias. Its corrected setup fails at **0.048560187** when
clock realignment is removed, and passes at **0** restored; no tolerance change.

**Full BUS parity remains open.** A faster 10:1 attack was rejected because it
improved bursts while worsening programme RMS. Fixed 4:1 programme max improves
0.188057 → 0.169447 dB, but 2:1 worsens 0.125663 → 0.159622 dB and 10:1 worsens
0.839825 → 0.859614 dB. All 60 frequency holdouts remain under 0.5 dB (max
0.440184); static 72/171-cell maxima are 0.226143/0.222995 dB. No parity bounds,
static-law tables, attack/RC constants, golden expectations, parameters or UI
were changed. Auto programme is unchanged; its quiet gain bias is now a
measured remaining surface. Charging/depth, external UAD calibration,
saturation/nonlinear waveform and meter/fade limits remain unfinished.

All seven native sibling modes (including OPTO/FET/VCA), 36 external cases,
mono/stereo Auto, dry and listen renders are byte-identical to baseline. Bypass
is exact after its 30 ms crossfade (initial max difference 7.858034e−7). Installed
AU/VST3/core recovery and fixed/Auto programme renders match sample for sample.
Native reference renders use five seconds of silent preroll; match that in
measurement probes. Direct native/core identity checks explicitly use zero
preroll on both sides. Do not conflate cold and settled bias state.

Release build and automatic installation completed; AU/VST3/CLAP/LV2 full
manifests match and signatures verify. Auval passes; pluginval strictness 5
(including GUI) reports SUCCESS. Full CTest initially passed 8/9 (269.00 s);
the only failure was the phase-test setup above. Its corrected suite passed
**1/1 in 33.42 s**; all nine suites now have passing results.
Installed AU SHA-256:
`519348c0932b4e33a88c7bc4f532163aebebac2fd92b23c77e6be03ca8a12ba1`.
Artifacts: `build-multi-comp-1176/bus-release-20260909/` (baseline, captures,
fault logs, native checks, manifests and reports). Logic was left running;
save/restart to load the binary. No commits, version bump or publication.
Concurrent chord-analyzer work was preserved.

## BUS 2:1 knee and attack continuation — 2026-09-09

See [the current BUS 2:1 report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-knee-2026-09-09.md).
A measured knee correction plus 2:1 attack calibration reduces fresh independent
burst RMS **0.273732 → 0.075829 dB**, maximum **1.165215 → 0.180074 dB**.
Independent threshold −5 maximum is **0.532948 → 0.115692 dB** at shipping 2×;
HR4/HR28 maximum is **0.509081 → 0.132767 dB**. The previous 2:1 frequency
outlier improves **0.657824 → 0.322223 dB**. All-ratio frequency holdouts now fit
a tightened 0.5 dB guard (maximum 0.433421 dB).

Curve-only correction was rejected because it accelerated early attack. A fresh
control exposed a pre-existing 1.09 dB error in the first-millisecond window.
The final 2:1 attack constants are calibrated separately; 4:1/10:1 and external
coefficients are unchanged. New constants are empirical, not UA circuit values.
Original 2:1 Auto maximum improves 0.596074 → 0.335716 dB and independent Auto
0.649324 → 0.403647 dB. All static/dynamics gates pass without widening bounds.

**Full BUS parity remains open.** The 2:1 Auto programme worsens in level error
−0.032500 → +0.055820 dB and frame maximum 0.147607 → 0.191258 dB despite improved
correlation. The fixed 2:1 programme improves; 4:1/10:1 programme scores are
unchanged. Preserve 0.613556 dB fixed-level, 0.619530 dB independent Auto and
0.839825 dB programme-frame residuals, external calibration, saturation/nonlinear
waveforms and meter ballistics. New transient holdouts are at 48 kHz only.

New dense/transient guards and tightened frequency guard all failed on baseline
and passed after restoration. Curve/attack faults fail separately; a ratio-leak
fault fails an exact programme comparison (the looser frequency gate did not
detect it). The default 2:1 BUS golden RMS changed 0.026390437 → 0.024968266;
the old DSP fails the revised expectation and the final DSP passes. Peak and
other mode expectations are unchanged. No faulty plugin was installed.

Seven sibling native renders and 36 external comparisons are byte-identical.
Dry/listen are unchanged; bypass differs by at most 1.49e−8 during its existing
initial 30 ms fade, then matches exactly. Installed AU dense/transient/Auto
renders are byte-identical to the core. UI, IDs/state, presets and fade are
unchanged; the fade holdout remains within 0.097216 dB.

**9/9 CTest passed (245.00 s)**, auval passed and pluginval strictness 5 passed.
All four formats are installed and match full manifests; strict signatures verify.
Latest AU executable SHA-256:
`0260dbc757671804217bcb11aa2493aa438bd234c9490ac043f4c826b0e5b3df`.
Artifacts: `build-multi-comp-1176/bus-knee-20260909/`. Save and restart Logic for
testing; it was left alone. Work is uncommitted.

## BUS detector frequency continuation — 2026-09-09

See [the current BUS frequency report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-frequency-2026-09-09.md).
Native UAD phase/detune controls support host-rate internal feedback detection.
BUS now updates its internal control once per host sample while retaining audio
oversampling; external detection keeps its original oversampled behavior.
The 57-case 44.1/48/96 kHz RMS is **0.369713 → 0.170514 dB**, maximum
**1.908289 → 0.287907 dB** at shipping 2×. Independent frequency/level/ratio RMS
is **0.579708 → 0.282004 dB**, maximum **2.535667 → 0.657824 dB**. The earlier
unexplained 8 kHz outlier is superseded by this detector sampling work.

**Full BUS parity remains open.** Independent 2:1 frequency RMS worsened
0.398846 → 0.419603 dB despite improved maximum. Some detuned points worsen too.
Current Auto and fixed-level maxima are 0.649324 and 0.613556 dB. The fixed 10:1
programme maximum is 0.839825 dB; overall level error worsened to −0.068455 dB.
Keep external calibration, nonlinear waveform/saturation and meter ballistics
open. The existing static, attack and Auto gates still pass without coefficient
changes. UI, fade, parameters, state, presets and sibling-mode DSP are unchanged.

The new frequency tests cover 1×/2×/4×, three sample rates and mono/stereo.
Both new reference assertions failed on baseline and passed after the fix;
five isolated faults also failed and passed after restoration. New clock state
is cleared on reset/prepare and realigned on rate changes. Seven native sibling
renders, twelve external comparisons, and mono/stereo 1×/bypass/dry/listen renders
are byte-identical to baseline. Installed AU phase and Auto-frequency audio is
byte-identical to the final core. The fade holdout remains within 0.097216 dB.

**9/9 CTest passed (245.49 s)**, auval passed and pluginval strictness 5 passed.
All four formats are installed and match complete build manifests; strict
AU/VST3/CLAP signatures verify. Latest AU executable SHA-256:
`5d56447e22b0a12ab0e9d038c0a3f5b177457b309d718be5f86738b9870e1417`.
Artifacts: `build-multi-comp-1176/bus-frequency-20260909/`. Save and restart Logic
for testing; it was left alone. Work is uncommitted.

## BUS Auto release continuation — 2026-09-09

See [the latest BUS Auto report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-auto-2026-09-09.md).
The slow reservoir now couples continuously to the fast control envelope,
including initial recovery, with a shared **600 ms charge / 90% storage target**.
Fast/slow decay rates are unchanged. Original Auto recovery RMS **0.561481 →
0.242598 dB**, maximum **1.167166 → 0.595128 dB**. Its permanent guard is now
0.6 dB for all three ratios. New independent burst maximum **1.977616 →
0.648983 dB** (explicit new 0.7 dB bound); 44.1/96 kHz maximum 0.618175 dB.
Three fresh Auto programme level errors are below **0.029 dB**, down from
as much as 1.060 dB; 10:1 frame RMS **1.673030 → 0.125694 dB**.

**Full BUS parity remains open.** Independent Auto cases still exceed 0.6 dB;
10:1 / 30 ms maximum slightly worsened **0.614345 → 0.620351 dB** despite better
RMS. Keep the previous fixed-release level/programme residuals, 8 kHz outlier,
external calibration, nonlinear waveform and meter-ballistic work open.

Twelve fixed-release cases, twelve external-detector cases and seven native
sibling-mode renders are byte-identical to baseline. Installed AU Auto programme
and long recovery renders are byte-identical to the core. UI, fade, IDs/schema,
presets, static law and golden expectations are unchanged. The original fade
holdout remains within 0.097216 dB. Both changed/new fixture groups failed on
baseline and passed after the fix; three isolated faults failed and passed after
restoration. No faulty plugin was installed.

**8/8 CTest passed (213.92 s)**, auval passed and pluginval strictness 5 passed.
All four formats are installed and match full build manifests; strict signatures
verify. Latest AU executable SHA-256:
`966311021f5113a427b932f7b78063c9ec5cd2d8d84a065c7d2f4fbbd9318351`.
Artifacts: `build-multi-comp-1176/bus-auto-20260909/`. Save and restart Logic for
testing; it was left alone. Work is uncommitted.

## BUS attack continuation — 2026-09-09

See [the latest BUS attack report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-attack-2026-09-08.md).
Continuous discharge during charging fixes the major slow-attack discrepancy.
All-ratio timing across 28 captures: **0.661225 → 0.205780 dB RMS**, maximum
**3.316294 → 0.562096 dB**. Independent phase maximum 0.509702 dB; independent
levels 0.612459 dB (explicit 0.65 dB bound); 44.1/96 kHz slow-attack maximum
0.337673 dB. Internal attack RC constants are shared across ratios. The charge
drive cap is removed; only the 2:1 static bias changed, by −0.15 dB.

Programme overall level errors are now below 0.060 dB. **Full BUS parity remains
open:** Auto tails still reach 1.167166 dB; the 10:1 programme worst 50 ms frame
grew from 0.736055 to 0.843230 dB despite improved aggregate scores. Preserve
these residuals, the 0.612459 dB level holdout and −0.630486 dB 8 kHz outlier.
External calibration, nonlinear waveform behavior and meter ballistics remain.
The previously documented 3.316294 dB slow-attack failure is superseded by this
pass, and the fade holdout remains within 0.097216 dB.

Seven sibling native renders and twelve external-detector compatibility cases
are byte-identical to the saved baseline. Installed AU/VST3 matched-control BUS
renders are byte-identical. UI, fade, IDs/schema and presets are unchanged.
All **8/8 CTest passed (212.77 s)**; auval and pluginval strictness 5 passed.
Four new fixture groups failed on baseline; six deliberate faults failed and
passed after restoration. No faulty plugin was installed. All four restored
formats are installed and match complete build manifests; strict signatures
verify. Latest AU executable SHA-256:
`055366eba290307490a2742088297ff7feffc7c6f34e463b60975e5046676627`.
Artifacts: `build-multi-comp-1176/bus-attack-20260908/`. Logic was left alone;
save and restart it for testing. Work is uncommitted.

## BUS fade continuation — 2026-09-08

See [the latest BUS fade report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-fade-2026-09-08.md).
The previously failed 3.5-second holdout is now **0.097216 dB maximum error**,
down from 13.9901 dB. Six full fade-out/in rates, five unseen intermediate rates,
44.1/48/96 kHz captures, reversal and live rate changes support an empirical
float ramp with two clock ranges and a measured attenuation table. Across
24,863 windows from −1 to −60 dB, **12.451035 → 0.026192 dB RMS**, final worst
0.371896 dB. Preserve float accumulation: a double mutation fails at 1.683021 dB.

All eight native mode renders are byte-identical with fade off. The installed AU
and built VST3 also produce byte-identical fade-automation audio. The approved UI,
compressor laws, IDs and state schema are unchanged. Full **8/8 CTest passed
(192.84 s)**, auval and pluginval strictness 5 passed. Four deliberate faults
failed and passed after restoration; the original fade implementation also failed
the new reference assertion before the fix. All four formats are installed and
match complete build manifests. Latest AU SHA-256:
`d9dca9b460b096ca795488351472973db879f88ca50b20faf3a7c412e78297c8`.

**Full BUS parity remains open:** slow 30 ms attack (2:1 worst 3.316294 dB),
external calibration, the 8 kHz detector outlier, saturation and meter ballistics.
Fade below −60 dB, other sample rates and shift-click instant reset are outside
this gate. The fade failure mentioned in older entries below is now superseded;
other failed audio holdouts must still be preserved.
Artifacts: `build-multi-comp-1176/bus-fade-20260908/`. Work is uncommitted.

## BUS dynamics continuation — 2026-09-08

See [the current BUS dynamics report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-dynamics-2026-09-08.md).
Internal control now integrates rectified gain reduction in dB; fixed releases
use measured constants and Auto has a slow reservoir. Removed the unconditional
60 Hz detector poles; BUS coupling poles are locally calibrated to 1.67 Hz.
Repeated same-rate host prepare now clears all BUS histories via clearBusState().

Original timed error **2.478466 → 0.422579 dB RMS**. Untouched Auto holds across
three ratios: **3.871825 → 0.572964 dB RMS**. Three programme holdouts have
**<0.39 dB overall RMS error, >0.999 correlation**. Static gates remain below
0.6 dB, with final 171-cell maximum 0.537220 dB. Seven sibling native renders
remain byte-identical. UI/parameter IDs/state schema are unchanged.

**Full BUS parity remains open:** slow 30 ms attack (held-out 2:1 worst 3.316294
dB), 8 kHz detector behavior (~0.655 dB outlier; cause not proven), external
calibration, saturation, meter ballistics, and the existing **13.9901 dB fade
holdout**. Do not claim full parity or silently drop failed holdouts.

Final 8/8 CTest (192.49 s), auval and pluginval strictness 5 pass. Eight deliberate
faults produced twelve failing/restored passing invocations. All four formats
are installed and match full build manifests. Latest AU SHA-256:
`c88253af7fd6ea446144c41447dd5b0fcfbddadaf1fb0a8e197a815b5eeb7feb`.
Artifacts: `build-multi-comp-1176/bus-dynamics-20260908/`. Work is uncommitted.

## BUS static calibration continuation — 2026-09-08

See [the current BUS audio report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-audio-2026-09-08.md).
The internal feedback law is now calibrated at attack 0.1 ms / release 0.1 s:
72-cell RMSE **11.725892 → 0.109223 dB**; 171 untouched cells **10.154552 →
0.102698 dB**, worst **0.440956 dB**. HR4/HR28 holdouts also pass. The external
feedforward law is intentionally preserved; all seven sibling modes remain
unchanged. The approved BUS UI is unchanged. A working AU is installed; all
seven native suites, auval and pluginval strictness 5 pass. Final AU SHA-256:
`f33dc67bb54ef157786892b02e9bd1ee270a93d8a71b5b53ad3f9e1179a17947`.

**BUS full parity is still open.** Attack/release remains coupled to sustained
compression: the 30 ms attack holdout over-compresses by **4.256688 dB**.
Corrected transient analysis requires measured delays (UAD 86 / candidate 27
samples) and rejection of filter tails, not raw unaligned RMS. A simple peak
follower was tried and rejected because it worsened fast-attack and Auto-release
gates. Do not infer that topology is proven. Preserve the previous **13.9901 dB**
failed fade holdout. External reference calibration, frequency/programme,
saturation, and meter-ballistic parity still need measurement.

Artifacts: `build-multi-comp-1176/bus-audio-20260908/`. No commit was made.

# Multi-Comp 2 / Opto (LA-2A) — handoff, 2026-08-22 snapshot

**Latest functional continuation (2026-09-08):** OPTO GR/+10/+4 and FET
GR/+8/+4/OFF now work. Display parameters append at IDs 103/104; state v6
migrates v3/v4/v5 without moving earlier IDs. See the
[meter-control report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-vintage-meter-controls-2026-09-08.md).
Seven suites pass (178.67 s), 400 render cases pass, five audio fixtures are
byte-identical, and the installed AU restores meter selections. AU/VST3/CLAP
signatures and auval pass. Current installed AU SHA-256:
`18dde3cd60b21c667b7b78b88576d316326384c237b0c417c4ab9bdf59406f4c`.
The GR-only notes below are superseded. BUS audio parity remains open; its
existing source-only unit-label cleanup was picked up by this build.

**Latest UI refinement (2026-09-08):** OPTO and vintage FET now use revised
1120 × 380 UAD-informed faces. See the [UI report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-opto-fet-ui-2026-09-08.md)
for fresh installed-AU screenshots, failing/restored checks, and limitations.
VCA, BUS, and Studio FET screenshots are pixel-identical to their pre-task
captures. DSP/parameters/state were unchanged. The new installed AU SHA-256 is
`ef8342defc46a751495a6c52ba5ad20a6f6f9cf7fc57f693013e15ad169e2c1d`;
strict signature verification and auval pass. Logic was left running.
OPTO/FET remain GR-only; FET's +8/+4/OFF bank is still decorative.

**Current BUS continuation:** see [the 2026-09-08 BUS report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-bus-parity-2026-09-08.md). The user targets the UAD SSL G Bus Compressor. The horizontal 1120 × 380 faceplate and new controls are implemented; audio parity remains open. Use the corrected `--disable-aux-inputs` reference captures, and retain the failed 3.5-second fade holdout.

**Local installation follow-up (2026-09-08):** the owner authorized automatic
installation after every new local plugin build. `build-multi-comp-1176` now has
`DUSK_DAF_INSTALL_LOCAL=ON`. That BUS-stage AU was installed in the user's Components
directory; all four bundle files match the build, its signature verifies, and
`auval -v aufx DsMc Dusk` passes. The executable SHA-256 is
`493fd1165e8f150c0d08f625f3f21485a1eee3c1371d3f7b7b45e7f25f86d78b`.
The older installed AU was backed up under the BUS artifact directory. See
AGENTS.md / CLAUDE.md for the standing installation preference; do not ask again.

**Prior three-mode continuation:** see §16 and
[the 2026-09-07 parity report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-parity-2026-09-07.md).
The owner's current request covers Opto, FET and VCA; the Opto-only scope below
describes the historical session. The current work remains uncommitted.
...
## 16. Three-model parity continuation — 2026-09-07

See [the current review and measurement report](../dusk-audio-tools/plugins/MultiComp/handoff/reports/multi-comp-2-parity-2026-09-07.md).
This checkout lacked the VCA work through `mc2/dbx160-vca-ui` / `c1387d2`;
its plugin changes are now present uncommitted, plus a measured VCA output
headroom fix and strict version-3 state-key validation. Fresh UAD comparisons
cover all three models. Native and no-FMA CTest pass 4/4; the full native Linux
ARM64 core suite and pluginval strictness 5 pass.

All three models now pass the measured audio-parity gates. Opto's full recovery
grid, settled Compress/Limit frequency surface and paired dense programme are
gated: worst frequency error 0.490450 dB across 122 cells; worst early recovery
point 0.980976 dB across 20 cells; all four paired programme rows have RMS
<0.5 dB and correlation >0.9. VCA detector/audio alignment reduces the measured
oversampling spread from 0.209960 to 0.000010 dB. Final fresh FET/VCA static
maxima are 0.095826/0.051007 dB. All 80 FET and 125 VCA final candidate renders
are sample-identical to their accepted candidate baselines.

Final VST3 SHA-256 starts `a2988156308a`. Native/no-FMA CTest pass 4/4 in
181.07/197.07 seconds; Linux ARM64 core and pluginval strictness 5 pass. The
report includes 81 observed Opto assertion failures against the old model,
five failing reset mutants and restored passes. The independent programme's
lowest correlation is 0.896417 despite RMS <0.5 dB; exact waveform parity is
not claimed. The legacy v3 VST3 host-cache/display issue remains (migrated
audio and re-saved state are correct), and visual meter parity is unverified.
Do not reuse historical §3 residuals or report-only recovery laws as current
parity gates; use the repository reference fixtures and the current report.
No branch switch, commit, install, release or change outside this checkout was
performed. Artifacts and writable campaign copies are under
`build-multi-comp-1176/parity-20260907/`.
