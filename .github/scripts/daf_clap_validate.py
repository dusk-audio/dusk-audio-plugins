...
violating the CLAP spec. This script runs clap-validator on a built .clap and:
  ...
  HARD FAIL  If the validator reports the 'clap_host_latency::changed' host
             callback error, DAF has regressed to the upstream bug -> exit 1.
...
Usage: daf_clap_validate.py <path-to-plugin.clap>
Env:
...
def main():
    if len(sys.argv) != 2:
        log("usage: daf_clap_validate.py <path-to.clap>")
        sys.exit(2)
    clap = sys.argv[1]
    ...
    try:
        ...
        # HARD GATE: the DAF CLAP latency contract.
