...
- `plugins/tape-echo/daf-plugin/` — DAF shell + ImGui UI + CMake
- branch `tape-echo/daf-core` — full commit history of the migration; read the
...
1. **Framework-free DSP core**: one class, pure C++17, zero JUCE/DAF includes.
...
2. **Thin DAF shell** (`<Name>Plugin.cpp`): param table in `initParameter`,
...
## Reusable building blocks (extract per 01-shared-daf-extraction.md)
...
| Chrome knob / LED / VU / toggle / preset combo / crisp font loader | `daf-plugin/TapeEchoUI.cpp` | LookAndFeel classes |
| Weak-symbol meter bridge | `daf-plugin/TapeEchoAccess.hpp` + `TapeEchoPlugin.cpp` | editor timers reading the processor |
...
9. **pluginval timeout**: use `--timeout-ms 120000`; the 30 s default
   ...
   **pluginval editor tests under headless Xvfb are unreliable**: pluginval's
   ...
   reparenting ANY DAF plugin's editor window (verified identical crash frame
   across sunset-circuits / multi-q / tapemachine DAF builds — not a plugin bug).
...
1. **A/B against the JUCE build** — the killer step. The hosted render tool
   ...
   Render the existing JUCE VST3 and your DAF VST3 with identical input and
...
- One branch per plugin: `<slug>/daf-core`. Commit granularly with detailed
  messages (follow the tape-echo branch's style). No Claude co-author
  trailers. Do not push without being asked.
- The JUCE version stays installed and buildable — DAF versions are new
  plugin IDs; users' sessions need the old ones. Never delete JUCE targets.
- **Naming policy (decided)**: DAF builds ship as versioned successors of
...
1. `01-shared-daf-extraction.md` (prerequisite for everything)
