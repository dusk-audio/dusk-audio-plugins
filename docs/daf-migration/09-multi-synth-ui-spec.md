...
`daf-plugin/MultiSynthUI.cpp`. It expands the "UI spec" section of `09-multi-synth.md`
...
- Fonts via `duskdaf::loadCrispFontSet` at design sizes `× getScaleFactor()`; bold TTF
...
- Reuse `duskdaf::DuskPanel` (knob/LED/toggle/text/valueBubble/valueEdit/curvePoint) and
  `duskdaf::RealFFT` from `DuskImGuiWidgets.hpp`. New widgets in this spec are additions
...
maps to exactly one on-screen control. The DAF param enum is generated from the X-macro
...
is **tracked centrally**, in the `duskdaf::ParamHost` overrides, not left to the widget:
...
Store as a `Palette` per mode (extend `duskdaf::Palette` with `background`, `panel`,
...
- **Loading** goes through `applyCombined()` — the identical entry point ◀/▶ use, dispatching
  ...
  a DAF UI cannot ask the host to load a program, so the preset's parameters are pushed
...
The DAF DearImGui backend has no `VtxOffset` support, so **one window's draw list corrupts
