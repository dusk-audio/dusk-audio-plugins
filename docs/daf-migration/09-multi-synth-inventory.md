# Multi-Synth JUCE → DAF Port: Read-Only Inventory
...
DAF target pattern (from tape-echo template): a framework-free `TapeEchoDSP` class with `prepare(sr, maxBlock)`, `reset()`, `processBlock(in, out, nCh, nSamples)`, atomic `setX()` setters, `getOutputLevel()`. Params live in a shared `TapeEchoParams.hpp` enum + factory-preset table. Shared DSP primitives in `plugins/shared-daf/dsp/` (`DuskSmoothed.hpp`, `DuskFilters.hpp`, `DuskOversampler.hpp` with `HalfbandFIR`). The port should mirror this: a `MultiSynthDSP` engine class + `MultiSynthParams.hpp`.
...
> **Note on code identifiers:** literal symbols such as `JunoChorusEffect` and `junoChorus` are quoted verbatim from the original JUCE sources this inventory documents; the DAF port renamed them. They are kept as-is here so the inventory stays factually accurate about the JUCE code.
...
- **`ReverbEffect`** — wraps **`juce::Reverb`** (Freeverb). Pre-delay circular buffer (≤200 ms). Maps decay→roomSize. **This is the one nontrivial external DSP dependency to replace for DAF.**
...
- **MultiSynthVoice.h**: `juce::SmoothedValue<float>`, `juce::Random`, `juce::MidiMessage::getMidiNoteInHertz`, `jlimit`, `jmax`, `MathConstants::halfPi`. STL: `array`, `pow/log2/abs/sqrt/cos/sin/isnan/isinf`. → `SmoothedValue` maps to shared-daf `DuskSmoothed.hpp`; `getMidiNoteInHertz` = `440·2^((n−69)/12)`; `Random` → any PRNG.
...
- **MultiSynth.h/.cpp**: full `juce::AudioProcessor`, `AudioProcessorValueTreeState`, `AudioParameterChoice/Float/Int/Bool`, `ParameterID`, `NormalisableRange`, `StringArray`, `AudioBuffer<float>`, `MidiBuffer`/`MidiMessage`, `ScopedNoDenormals`, `Decibels`, `PlayHead`/`getPosition`/`getBpm`/`getIsPlaying`, `XmlElement`/`MemoryBlock`/`ValueTree`, `std::atomic`. → all replaced by the DAF `Plugin` shell + `MultiSynthParams.hpp`.
- **Editor**: `juce_gui_basics` entirely (see §8) — replaced by a DAF/DGL UI.
...
Every preset first resets a common baseline (masterVol −6 dB, oversampling→2×, unison 1, FX off, etc.) then overrides. Presets set `oversampling`→1 (2×) — meaning **all factory presets ship at the buggy default OS rate** (§9). For DAF, convert to a static preset table (mirror tape-echo's `kFactoryPresets` value-array form).
...
- DAF note: factory presets should become DAF `Program`s or an internal table.
