# 09 — Multi-Synth: JUCE → DAF port + design finalization
...
Multi-Synth DAF port. The companion file `09-multi-synth-inventory.md` is the exhaustive
...
`MSyn` VST3 code, collision-grepped against every repo DAF `d_cconst`): VST3 code `DsSC`,
...
├── core/                     # framework-free C++17, zero JUCE/DAF includes
...
Reuse `plugins/shared-daf/dsp/` (`DuskSmoothed`, `DuskOversampler` HalfbandFIR,
...
1. **Oversampling redesign.** Voices render at `hostRate × osFactor` (1/2/4, default 2×),
   ...
   (shared-daf), NOT box-average. This fixes the octave-drop bug. Envelope/LFO/arp time
...
`acidSlideTime`) + 48 step params (`seqPitch/Accent/Slide 0-15`). DAF param table
is generated from `daf-plugin/tools/gen_params.py` so shell + UI + presets share it.
...
2. **A/B vs JUCE build at 4× only** (the only correct JUCE setting): render identical
   note sequences (JUCE via `duskverb_render` VST3 hosting, DAF likewise), match
...
- **P3** Integration (SynthMode 4/5 into Voice/params) + DAF shell + programs
- **P4** ImGui UI
