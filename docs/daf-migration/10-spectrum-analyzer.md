# Handoff: Spectrum Analyzer → DAF

> Prompt for the executing agent: Read `docs/daf-migration/00-OVERVIEW.md`
> first. Execute on branch `spectrum-analyzer/daf-core`. This is the
...
building the DAF table; the JUCE layout is the single source of truth, and
the A/B gate below diffs the dumped tables.

State: plain APVTS XML under tag `SpectrumAnalyzerState`, no custom
properties, no file references. The DAF port needs only the standard
...
- Successor: product **"Spectrum Analyzer 2"**, DAF dir
  `plugins/spectrum-analyzer/daf-plugin`, base `spectrum_analyzer_2`,
  AU subtype **`DsSp`** (free: fleet uses DsTM/DsFq/DsTE/DsMq/DsSC),
  type `aufx`, LV2 URI
  `https://dusk-audio.github.io/plugins/spectrum-analyzer-2`, CLAP id
  `com.dusk-audio.spectrum-analyzer-2`. Register in daf-build.yml's plan
...
1. **FFT without JUCE**: DONE (PR #194). `FFTr2` was promoted VERBATIM from
   ...
   `plugins/shared-daf/dsp/DuskFft.hpp`: same `duskaudio` namespace, same
   ...
   `MultiQMatch.cpp` still holds the entire Match engine and the DAF target
   keeps compiling it; only the FFT left that file. Guards that ran on the
   move: MultiQMatchTest 21/21, JUCE AU + DAF vst3/clap/lv2/au builds, full
...
1. Promote `FFTr2` to shared-daf; re-run Multi-Q parity (guard).
2. Extract `core/` (SpectrumCore + the five meters), framework-free; port
   ...
   both builds.
3. DAF shell: 10-param table, passthrough, state, rings.
4. ImGui editor.
