...
> `tapemachine/daf-core`. Requires shared-daf + ideally the 4k-eq pilot.
...
- 4–6× `juce::dsp::StateVariableTPTFilter` → write a shared TPT SVF
  (Cytomic/Zavalishin topology, ~30 lines; add to `shared-daf/dsp/`,
...
6. Latency from oversampling reported via DAF latency support; bypass
...
- Add anything generic you build (TPT SVF, dual VU layout) to `shared-daf`.
