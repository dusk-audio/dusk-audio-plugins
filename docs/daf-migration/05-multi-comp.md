# Handoff: Multi-Comp → DAF

> Prompt for the executing agent: Read `docs/daf-migration/00-OVERVIEW.md`
> first. Execute on branch `multi-comp/dpf-core`. Requires shared-daf.
...
- 22× `juce::dsp::IIR` (sidechain filters, crossovers) → shared Biquad;
  ...
  biquads in `shared-daf` and verify flat-sum reconstruction.
...
- The JUCE version's oversampling parameter accepts only '2x'/'4x' strings
  ...
  since DAF version is a new plugin identity anyway.
- v1.3.1 fixed bypass-related DAW instability (repo commit `7ff0ad9`) — the
  DAF bypass must use the designation-param + crossfade pattern and be
