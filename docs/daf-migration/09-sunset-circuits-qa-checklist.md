...
runs in CI. The `test-linux` job in `.github/workflows/daf-release.yml` builds
...
`plugins/sunset-circuits/daf-plugin/tools/lv2_smoke.c`, the same library `jalv` uses).

Results:

- Bundle loads; plugin resolves by URI `https://dusk-audio.github.io/plugins/sunset-circuits`.
- `instantiate` + `activate` + `run` all succeed (the DAF options feature and a
...
This is the authoritative 223-parameter save/restore proxy at the DAF/VST3 level
