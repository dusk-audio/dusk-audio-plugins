# Handoff: Convolution Reverb → DAF (hardest — do last)

> Prompt for the executing agent: Read `docs/daf-migration/00-OVERVIEW.md`
> first. Execute on branch `convolution-reverb/daf-core`. Requires
> shared-daf and experience from at least two prior ports.
...
1. **Partitioned convolution engine** (replaces `juce::dsp::Convolution`):
   - Use **FFTConvolver** (github.com/HiFi-LoFi/FFTConvolver, MIT) — small,
     ...
     `shared-daf/third_party/`. Alternative: hand-rolled uniform-partition
   ...
   - Latency: zero-latency partitioning or report block latency via DAF —
...
3. **Async IR loading** (CLAUDE.md pattern applies): load/resample on a
   worker thread; never allocate or load on the audio thread. DAF has no
...
4. **State**: IR file path must persist → `DISTRHO_PLUGIN_WANT_STATE 1` +
   ...
   evaluate it vs an ImGui browser; prefer DAF's host-integrated dialog.
